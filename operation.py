...
'''
  #bind the class we create to the table in our database
  Base.metadata.bind = engine
  #a link of coummunication between the code will excuted
  # and engine we just created
  DBSession = sessionmaker(bind = engine)
  #slqalchemy operation SQL command via interface session
  #session allow to exceute all commend on the database,
  #but change will be persisted after we commit
  session = DBSession()
  #to do something in database, just call the method of session
  firstRestaurant = Restaurant(name = "Pizza Palace")
  #add to the staging zone of database
...
bugers = session.query(MenuItem).filter_by(name = 'Veggie Burger')
for each in bugers:
    print each.id
    print each.price
